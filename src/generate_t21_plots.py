...
def plot_model_scaling_metrics(out_dir: Path) -> None:
    ...
    ax.set_ylim(0, 72)
    ax.set_title(
        'Model Size Comparison — "Describe this image in a short sentence." (no thinking)',
        pad=15,
    )
...
def plot_model_scaling_time(out_dir: Path) -> None:
    ...
    fig.suptitle(
        "Inference Cost vs Model Size (no thinking)",
        fontsize=18,
        fontweight="bold",
        y=1.02,
    )
...
def plot_thinking_vs_no_thinking(out_dir: Path) -> None:
    ...
    for col, metric in enumerate(METRICS):
        ax = axes[col]
        all_vals = [pct(NO_THINK[s][metric]) for s in sizes] + [
            pct(THINK[s][metric]) for s in sizes
        ]
    ...
    fig.suptitle(
        "Thinking Mode vs No Thinking — Metric Comparison",
        fontsize=18,
        fontweight="bold",
        y=1.03,
    )
...
def plot_thinking_time_penalty(out_dir: Path) -> None:
    ...
    for i, (bar_n, bar_y, mult) in enumerate(zip(bars_no, bars_yes, multipliers)):
        ax.text(
            bar_n.get_width() + 100,
            bar_n.get_y() + bar_n.get_height() / 2,
            f"{ms_no[i]:.0f} ms",
            va="center",
            fontsize=11,
            color="#334155",
        ...
        ax.text(
            bar_y.get_width() + 100,
            bar_y.get_y() + bar_y.get_height() / 2,
            f"{ms_yes[i]:.0f} ms",
            va="center",
            fontsize=11,
            color="#334155",
...
def plot_summary_table(out_dir: Path) -> None:
    ...
    for prompt, data in PROMPT_RESULTS.items():
        ...
        rows.append(
            [
                ...
                f"{pct(data['bleu1']):.1f}",
                f"{pct(data['bleu2']):.1f}",
                f"{pct(data['rougeL']):.1f}",
                f"{pct(data['meteor']):.1f}",
                f"{data['ms']:.0f}",
            ]
        )
    ...
    ax.axis("off")
    ax.set_title(
        "Task 2.1 — Full Results Summary", fontsize=22, fontweight="bold", pad=30
    )

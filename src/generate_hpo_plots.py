...
def plot_optimized_comparison(out_dir: Path) -> None:
    ...
    ax.set_ylabel("Score (%)")
    ax.set_title(
        "Optimised Baseline vs Optimised Best Architecture — After HPO (50 trials)"
    )
    ax.set_ylim(0, 80)
    ax.legend(loc="upper center", bbox_to_anchor=(0.5, -0.1), ncol=2)
    fig.tight_layout()
...
def plot_param_importance(
    sweep_dir: Path, study_name: str, color: str, out_path: Path, plot_id: str
) -> None:
...
def main(
    ...
    plot_hpo_history(
        ...
        best_trial=37,
        color=COL_C1,
        out_path=out / "05_c1_hpo_history.png",
        plot_id="5/6",
    )
    ...
    plot_hpo_history(
        trials_c2,
        "C2 (Best Architecture HPO)",
        best_trial=44,
        color=COL_C2,
        out_path=out / "06_c2_hpo_history.png",
        plot_id="6/8",
    )

    plot_param_importance(
        sweep_c1, "C1 — Baseline HPO", COL_C1, out / "07_c1_param_importance.png", "7/8"
    )
    plot_param_importance(
        sweep_c2,
        "C2 — Best Architecture HPO",
        COL_C2,
        out / "08_c2_param_importance.png",
        "8/8",
    )
